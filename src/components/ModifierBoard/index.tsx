...
const ModifierBoard = ({
  ...
  const noiseFileMap: Record<keyof IBackgroundNoise, string> = {
  ...
  const audioSettings: IAudioSetting[] = [
    {
      src: `${MUSIC_BASE}/city_traffic.mp3`,
      volume: backgroundNoise.cityTraffic / 100,
    },
    {
      src: `${MUSIC_BASE}/fireplace.mp3`,
      volume: backgroundNoise.fireplace / 100,
    },
    { src: `${MUSIC_BASE}/rain_city.mp3`, volume: rainValue / 100 },
  ];
  ...
  return (
    <>
      ...
      <div
        className={`modifier ${openMood ? "mood" : ""} ${openFocus ? "focus" : ""}`}
      >
          ...
          {openMood && (
            <div className="modifierBox">
              ...
              <div className="options">
                {(["sleep", "jazzy", "chill"] as MoodMode[]).map((mode) => (
                  <div
                    key={mode}
                    id={mode}
                    onClick={() => changeMoodHandler(mode)}
              ...
              <div className="volume">
                <Stack spacing={2} direction="row" sx={{ mb: 1 }} alignItems="center">
                  ...
                  <Slider
                    ...
                    value={volumeValue}
                    onChange={(_e, value) => changeVolumeHandler(value as number)}
                  />
              ...
              <div className="backgroundNoise">
                {(Object.entries(backgroundNoise) as [keyof IBackgroundNoise, number][]).map(
                  ([type, value]) => (
                    <div key={type} className="noise-option">
                      ...
                      <ReactAudioPlayer
                        preload="auto"
                        autoPlay
                        src={`${MUSIC_BASE}/${noiseFileMap[type]}.mp3`}
                        loop
                        volume={value / 100}
                      />
                      <Slider
                        ...
                        onChange={(_e, newValue) =>
                          changeNoiseHandler(type, newValue as number)
                        }
                      />
                    </div>
                  )
